...
public:
	...
	inline static void transfer(void *buf, size_t count) {
		...
		while (--count > 0) {
			uint8_t out = *(p + 1);
			while (!(SPSR & _BV(SPIF))) ;
			// MODIFICACIONES EN LA LIBRERIA:
			// -------------------------------------------------
			//uint8_t in = SPDR; <-- esto estaba sin comentar  |
			SPDR = out; //					   |

			//*p++ = in;  <-- esto estaba sin comentar	   |
			p++;	    //<-- esto NO estaba y lo he puesto yo |
			// -------------------------------------------------
		}
		while (!(SPSR & _BV(SPIF))) ;
		//*p = SPDR;    <-- esto estaba sin comentar	
	}
